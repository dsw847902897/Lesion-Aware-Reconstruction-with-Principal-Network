...
class postprocessor:
    def __init__(self):
        super().__init__()
        self.width = _C.IMAGE_SIZE1
        self.height = _C.IMAGE_SIZE1

    def __call__(self, cls_logits, bbox_pred):
        priors = priorbox()().to(cls_logits.device)
        batches_scores = F.softmax(cls_logits, dim=2)
        boxes = convert_locations_to_boxes(
            bbox_pred, priors, _C.ANCHORS_CENTER_VARIANCE , _C.ANCHORS_CENTER_VARIANCE 
        )
        ...
        for batch_id in range(batch_size):
            ...
            for class_id in range(1, per_img_scores.size(1)):  # skip background
                ...
                mask = scores > _C.TEST_CONFIDENCE_THRESHOLD
                scores = scores[mask]
                if scores.size(0) == 0:
                    continue
                ...
                keep = boxes_nms(boxes, scores, _C.TEST_CONFIDENCE_THRESHOLD, _C.TEST_MAX_PER_CLASS)
            ...
            if processed_boxes.size(0) > _C.TEST_MAX_PER_IMAGE > 0:
                processed_scores, keep = torch.topk(processed_scores, k=_C.TEST_MAX_PER_IMAGE)
