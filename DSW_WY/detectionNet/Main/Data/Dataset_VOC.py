...
from Transforms import transform,targettransform
...
class vocdataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        wh_dict=self.get_img_size(image_name)
        w=int(wh_dict['width'])
        h=int(wh_dict['height'])
        wh=[w,h]
        wh=torch.tensor(wh)
        ...
        return image, boxes, labels, image_name, wh
    ...
    def _get_annotation(self, image_name):
        ...
        for obj in objects: # .encode('utf-8').decode('UTF-8-sig') 解决Windows下中文编码问题
            ...
            class_name = obj.find('name').text.encode('utf-8').decode('UTF-8-sig').strip()
            if class_name != 'Odontoma':
                if class_name != 'OssifyingFibroma':
                    class_name='Cystic'
