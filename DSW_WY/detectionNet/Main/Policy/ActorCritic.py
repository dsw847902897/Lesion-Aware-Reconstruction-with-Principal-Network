...
from Policy.AC import PolicyNet, ValueNet
...
class ActorCritic:
    def __init__(self, actor_lr=_C.ACTOR_LR, critic_lr=_C.CRITIC_LR, gamma=_C.GAMMA, device=_C.DEVICE):
        ...
        if _C.AC_PICK:
            self.actor.load_state_dict(torch.load(os.path.join(_C.CURRENT_DIR,'Weight','trained','AC','actor_epoch5.pth')))
            self.critic.load_state_dict(torch.load(os.path.join(_C.CURRENT_DIR,'Weight','trained','AC','critic_epoch5.pth')))
